...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  function waga(agent) {
    const tonaz = parseFloat(agent.parameters.tonaz);
    const dlugosc = parseFloat(agent.parameters.dlugosc);
    ...
    if (tonaz && dlugosc) {
      ...
      if (tonaz < 4) {
        ...
        agent.add(`OK. ${tonaz} ton ${dlugosc} metrów waży ${wynikWaga} kilogramów.`);
        agent.add(odp);
      } else if (tonaz > 3) {
        let odp =
          'Nawinięto ' +
  ...
  function ustawLicznik(agent) {
    let tonaz = parseFloat(agent.parameters.tonaz);
    let dlugosc = parseFloat(agent.parameters.dlugosc);
    if (!tonaz) {
  ...
  function yourFunctionHandler(agent) {
    agent.add(`To jest widomość z funkcji chmury  Dialogflow's Cloud Functions od Firebase edytora!`);
    agent.add(
      new Card({
  ...
  function pakowanie(agent) {
    ...
    if (tonaz === 1) {
      ...
      agent.add(`1a banderoluj i spinaj `);
    } // 1 tona

    if (tonaz === 2) {
      if (dlugosc === 1) {
        agent.add(` 2 tony 1 metr  banderola jeden raz, pakuj po  sztuk i   spinaj.`);
      }
      if (dlugosc === 2) {
